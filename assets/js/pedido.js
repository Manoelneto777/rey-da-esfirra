...
const Pedido = (() => {
  const WHATSAPP_NUM = "5538991415141";
  ...
  function _renderizarResumo(itens) {
    ...
    // createElement + textContent: sem risco de XSS.
    container.innerHTML = "";
  ...
  async function _enviar() {
    ...
    try {
      ...
      setTimeout(
        () => _abrirWhatsApp({ nome, telefone, endereco, observacoes, itens, total }),
        800,
      );
    } catch (erro) {
  ...
  function _abrirWhatsApp({ nome, telefone, endereco, observacoes, itens, total }) {
    const linhasItens = itens
      .map(
        (i) =>
          `• ${i.nome} ×${i.quantidade} — R$ ${(i.preco * i.quantidade).toFixed(2).replace(".", ",")}`,
      )
      .join("\n");
    ...
    const msg = [
      ...
      .filter((l) => l !== null && l !== undefined && !(l === "" && !observacoes))
      .join("\n");
  ...
  function _limparFormulario() {
    ["pedidoNome", "pedidoTelefone", "pedidoEndereco", "pedidoObs"].forEach(
      (id) => {
