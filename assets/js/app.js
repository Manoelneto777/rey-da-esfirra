...
const Cardapio = (() => {
  ...
  function renderizar(produtos) {
    ...
    if (!grid) return;
    const catAtiva = document.querySelector('.cat-pill.active')?.dataset.cat || 'Todos';
    if (countEl) {
    ...
    produtos.forEach((p, i) => {
      ...
      btnAdd.addEventListener('click', () => {
        Cart.adicionar({ produto_id: parseInt(p.id), nome: p.nome, preco: parseFloat(p.preco), imagem: p.imagem });
...
const CartUI = (() => {
  ...
  function _renderizar({ itens, total, unidades }) {
    ...
    if (!itens.length) {
      ...
      setTimeout(() => document.getElementById('btnVoltarVazio')?.addEventListener('click', fechar), 0);
    ...
    if (footerEl) {
      ...
      document.getElementById('entregaValue').innerHTML = taxa === 0 ? '<span style="color:var(--neon);font-weight:700">Grátis 🎉</span>' : `R$ ${taxa.toFixed(2).replace('.', ',')}`;
      document.getElementById('totalValue').textContent = `R$ ${(total + taxa).toFixed(2).replace('.', ',')}`;
    }
  }
...
const Chatbot = (() => {
  ...
  function _toggle() {
    ...
    if (label) { label.style.display = aberto ? 'none' : 'block'; }
  ...
  function _addBotMsg(d) {
    ...
    el.innerHTML = _mdLite(d.texto || ''); _append(el);
  ...
  function _addTyping() { const el = document.createElement('div'); el.className = 'typing-indicator'; el.id = 'chatTyping';
    for(let i=0;i<3;i++){ const d=document.createElement('div'); d.className='typing-dot'; el.appendChild(d); } _append(el); }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (heroBtn) {
    heroBtn.addEventListener('click', () => {
      Cart.adicionar({ produto_id: 1, nome: 'Esfirra de Carne', preco: 7.50, imagem: 'carne.png' });
  ...
  document.querySelector('.mapa-card')?.addEventListener('click', () => window.open('https://www.google.com/maps', '_blank', 'noopener'));
});